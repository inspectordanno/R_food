...
foods <- c("coffee", "tea", "pancake", "ice cream", "french frie",
           "french peas", "apple", "banana", "strawberry", "pizza", "buffalo wings", "chicken wings", "mozzarella", "spaghetti", "chicken parmigiana", "flounder", "quiche")
...
food_over_time <- map_df(foods, function(food) {
  d %>%
    filter(year >= 1900 & year <= 1980  & location == "Waldorf Astoria") %>%
...
food_over_time %>% filter(food %in% c("chicken wings", "flounder")) %>%
  ggplot(aes(decennium, prop_food, color = food)) + food_time_plot
```
```{r}

food_over_time %>% filter(food %in% c("quiche", "spaghetti")) %>%
  ggplot(aes(decennium, prop_food, color = food)) + food_time_plot
```
```{r}

food_over_time %>% filter(food %in% c("mozzarella", "chicken parmigiana")) %>%
  ggplot(aes(decennium, prop_food, color = food)) + food_time_plot
```
